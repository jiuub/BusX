...
import com.xaau.bs.busx.SchemeBusStep;
...
public class BusSegmentListAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (position == 0) {
			...
			if (item.isWalk() && item.getWalk() != null && item.getWalk().getDistance() > 0) {
				...
				holder.busStationNum.setText((item.getBusLines().get(0).getPassStationNum() + 1) + "站");
	...
	private class ArrowClick implements OnClickListener {
		...
		private ArrowClick(final ViewHolder holder, final SchemeBusStep item) {
			mHolder = holder;
			mItem = item;
		}
